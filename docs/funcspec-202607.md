...
│       └── minoh-hiking-closure.geojson       # 通行止め・通行困難地点（フォールバック用同梱ファイル）
├── api/
...
> （nav ビュー自体はプレースホルダとして残置）。「設定と情報」は「設定」「情報」の2ボタンに分離した。

### 2.3 モーダル一覧
| モーダル | 用途 |
|----------|------|
| オフライン地図ダウンロード | タイルのダウンロード／キャッシュ削除、DL済みバージョン・サイズ表示 |
| 設定（home） | 地図表示トグル（時刻・緊急ポイント・ハイキングルート）、データ件数表示、マーカー/画像設定への導線 |
| 情報（home） | 起動時更新確認トグル、バージョン情報、メッセージ履歴、このアプリについて |
...
| 通行止め・通行困難地点 | 公開API（フォールバック: 同梱geojson） | ポイントマーカー（種別・理由・更新日ポップアップ） | ON（map ビュー時は常時表示） |
...
- 表示トグル（時刻・緊急ポイント・ハイキングルート）は「設定」モーダル内にある（→ [3.6](#36-設定モーダルの地図表示トグルデータ件数)）。
...
> 旧版（2026.2）ではマップ画面メニューにあった「時刻を表示」「緊急ポイントを表示」
> 「ハイキングルートを表示」のトグルは「設定」モーダルへ移動した（トグルの効果は従来どおり地図に反映）。

### 3.6 設定モーダルの地図表示トグル・データ件数
「設定」モーダル（ホームの「設定」ボタン）に、地図に何を表示するかのトグルとデータ件数を配置する。
...
| 緊急ポイントを表示 | トグル | ON |
| ハイキングルートを表示 | トグル | ON |
| データ件数（ポイント／ルート／スポット） | 表示 | 読み込んだ実データ件数を表示（未読込は `-`） |
| マーカーの設定 | ボタン | 設定モーダル（マーカー）を開く |
| 撮影画像の解像度の設定 | ボタン | 設定モーダル（画像、準備中）を開く |

- **データ件数** は表示トグルの ON/OFF に依らず、常に読み込んだ実データの件数を表示する。
...
- アプリシェルのキャッシュ名は `app-shell-<version>`（`SHELL_CACHE`。現行 `app-shell-2026-07-16.1`）。
...
- **情報モーダル:** 「バージョン情報」トグル ON のとき、地図とアプリのバージョンをサイト最新と
...
- 情報モーダルの「メッセージ履歴の表示」トグルで一覧表示（新しい順、`MM/DD HH:MM` +
...
通行止め・通行困難地点の**表示は全ユーザー向け**、**編集・公開は運用担当者向け**の機能である。
...
- データは起動時に**公開API（`/api/closures`）**から取得する。取得できない場合は同梱の静的ファイル
  `data/minoh-hiking-closure.geojson` にフォールバックし、オフライン時は SW の `closures-cache`
  （最後に取得した内容）を表示する。
- 情報モーダルの「バージョン情報」に、現在反映されている通行止めデータのバージョンを表示する。
...
- **公開:** 反映済みデータを公開API（`POST /api/closures`、公開トークン必須）へ送信し、全ユーザーへ公開する。
...
### 11.3 通行止め・通行困難地点（minoh-hiking-closure.geojson ほか）
- FeatureCollection（`version` 付き、各地点は Point）。同梱ファイルはフォールバック用。
...
| 通行止め公開API読込失敗 | 同梱静的ファイル→SWキャッシュの順にフォールバック（コンソール警告） |
...
| 2026-07-16 | 2026.3 | ホームメニューを刷新（「設定と情報」を「設定」「情報」に分離、「ハイキングコースの案内・ナビ」ボタンを除去）。地図表示トグル（時刻・緊急ポイント・ハイキングルート）をマップ画面メニューから「設定」モーダルへ移動し、データ件数（ポイント/ルート/スポット）表示を追加。精度円（現在地点を中心とする円）を常時表示から3秒間の一時表示に変更。通行止め・通行困難地点（表示は全ユーザー、編集・公開は運用担当者）を追加（詳細は運用手順書参照） |
