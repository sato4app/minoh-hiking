...
| closureClosed | 通行止め | #DC2626 | ✖ | 16 |
| closureDifficult | 通行困難地点 | #F59E0B | 三角 | 12 |
| trackStart | トラック開始点 | #000080 | 四角 | 12 |
| trackCurrent | トラック現在地点 | #000080 | 三角 | 16 |
| track | 移動記録 | #000080 | 線 | 4 |
...
- マーカーは種別（`kind`）ごとの既定スタイル: **通行止め（closed）＝赤✖（16px） /
  通行困難（difficult）＝橙三角（12px）**。色・形状・サイズはマーカー設定で変更できる（→ 7章）。
...
| 2026-07-18 | 2026.5 | マーカー設定: 「トラック」を「移動記録」に改称。通行止め・通行困難地点のマーカーを設定対象に追加（スポットの下に配置。既定: 通行止め＝赤✖ 16px／通行困難地点＝橙三角 12px。従来の固定スタイルは廃止）。マーカー形状に「✖」を追加 |
