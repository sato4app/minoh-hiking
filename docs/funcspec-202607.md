...
| バージョン情報等（home） | 設定（言語/Language・時刻表示トグル・起動時更新確認トグル）＋情報（バージョン情報〔各バージョン＋データ件数〕、メッセージ履歴、このアプリについて） |
...
| 項目 | 種別 | 既定 |
|------|------|------|
| 言語/Language | ドロップダウン | 日本語（`ja`）。English（`en`）を選択可。選択値は localStorage（`minoh-hiking.language`）に保存。**表示言語の切替は今後実装**（現状は選択値の保存のみ） |
| 時刻を表示 | トグル | ON |
...
- 「言語/Language」は選択値の保存のみで、英語表示への切替は未実装。
