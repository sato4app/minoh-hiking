...
あわせて、現在地の追跡と移動記録（移動経路の記録）機能を備える。
...
| バージョン情報等 | バージョン情報等モーダルを表示 |
| 言語/Language | ドロップダウン（「バージョン情報等」ボタンの下に配置）。日本語（`ja`）／English（`en`）を選択。選択値は localStorage（`minoh-hiking.language`）に保存し、リロードして全UI文言を選択言語で再表示する |
...
> 2026.6 で「バージョン情報等」に改称し、マーカー設定への導線はマップ画面メニューのみに変更、
> 「撮影画像の解像度の設定」は廃止した。2026.7 で「言語/Language」を「バージョン情報等」
> モーダルからホームメニュー（「バージョン情報等」ボタンの下）へ移動した。

### 2.3 モーダル一覧
| モーダル | 用途 |
|----------|------|
| オフライン地図ダウンロード | タイルのダウンロード／キャッシュ削除、DL済みバージョン・サイズ表示 |
| バージョン情報等（home） | 設定（時刻表示トグル・起動時更新確認トグル）＋情報（バージョン情報〔各バージョン＋データ件数〕、メッセージ履歴、このアプリについて） |
| マーカーの設定 | マーカーの色・形状・サイズ設定（マップ画面メニューからのみ開く） |
...
| 現在地・移動記録 | Geolocation API | 現在地マーカー・精度円・軌跡・各種マーカー | 「現在地点をマーカー表示」「現在地点は中央に表示」トグル連動（既定 ON）／軌跡は「移動経路を記録」連動 |

- オーバーレイはアプリ起動時にバックグラウンドで読み込み、map ビュー表示時のみ表示する。
  ホーム／ナビビューでは全オーバーレイを非表示にする。
- 「時刻を表示」トグルは「バージョン情報等」モーダル内にある（→ [3.6](#36-バージョン情報等モーダル)）。
...
| 現在地点は中央に表示 | トグル | ON |
| 移動経路を記録 | トグル | OFF |
| 統計表（地点数・移動距離） | 表示 | 記録中の地点数と移動距離(km)を表形式で表示（→ [4.4](#44-記録統計統計表終了時)） |
| 出力／クリア | ボタン | 移動経路の出力（**準備中**）・消去 |
...
「バージョン情報等」モーダル（ホームの「バージョン情報等」ボタン）の項目（上から順）:
...
- **バージョン情報**の内訳: 地図バージョン／アプリバージョン／通行止め・通行困難地点（バージョン）、
  および**データ件数**（ポイント・ルート・スポット・通行止めの件数を**横一列・中央寄せ**で表示。未読込は `-`）。
...
- 表示要素:
  ...
  - **移動記録現在地点マーカー:** 最終記録点（既定: 三角、進行方向に回転）
...
- **統計表:** 表示設定パネル内（出力・クリアボタンの左）に、現在の記録の
  `| 地点数 | nnn |`・`| 移動距離(km) | nn.mm |` を2行の表で表示する。
  パネルを開いたとき・記録点が追加されたとき・クリア時に更新される。
- **出力ボタン:** 移動経路の出力用（**機能は準備中**。押すと準備中の案内トーストを表示）。
- **記録終了時:** 記録中だった場合のみ、`記録地点 N 点 / 写真 N 枚 / 移動距離 N.NN km` の
  サマリをトーストとメッセージ履歴に出力する（軌跡が消去される前に統計を取得）。
...
- **「バージョン情報等」モーダル:** 「バージョン情報」トグル ON のとき、地図とアプリのバージョンをサイト最新と
...
- 「バージョン情報等」モーダルの「メッセージ履歴の表示」トグルで一覧表示（新しい順、`MM/DD HH:MM` +
...
- データは起動時に**公開API（`/api/closures`）**から取得する。取得できない場合（オフライン等）は
  ...
  （古い情報を出すより安全）。
- 「バージョン情報等」モーダルの「バージョン情報」に、現在反映されている通行止めデータの
  バージョンと件数を表示する。
...
- 移動経路の「出力」ボタンは準備中（押すと準備中の案内トーストを表示するのみ）。
- 「言語/Language」の切替対象はUI文言のみ。地図データの名称（ポイント名・スポット名・ルート名等）は
  言語によらず日本語のまま表示される。
- routeGuide / photoLocation マーカーはレイヤー未実装（設定のみ保持）。
- 移動記録（移動経路）はメモリ上の表示のみで、永続保存・エクスポートは未実装。
